#!/usr/bin/env bash
# RoboTwin adjust_bottle full EventValue-RL experiment.
#
# This is deliberately separate from the official πRL action-GAE script: the
# actor, Flow-SDE sampler, PPO loss, SFT checkpoint, episode length and seed
# files remain identical.  Only the learned event critic and signed credit
# estimator are enabled.  Branch steps are explicitly counted in logs; use a
# correspondingly reduced main-rollout epoch count when comparing fixed total
# simulator interaction budgets.
#SBATCH --job-name=robotwin_eventvalue
...
#SBATCH --output=/data/user/leviccdong/EKSF/outputs/robotwin_eventvalue_%j.out
#SBATCH --error=/data/user/leviccdong/EKSF/outputs/robotwin_eventvalue_%j.err
...
# 92 epochs is the approximate matched-main-rollout budget for the baseline's
# 100 epochs with four 10-step candidates every ten policy chunks.  Do not
# change this independently of the corresponding action-GAE control.
export ROBOTWIN_MAX_EPOCHS="${ROBOTWIN_MAX_EPOCHS:-92}"
...
export ROBOTWIN_LOG_PATH="${ROBOTWIN_LOG_PATH:-/data/user/leviccdong/EKSF/outputs/robotwin_eventvalue}"
export ROBOTWIN_EXPERIMENT_NAME="${ROBOTWIN_EXPERIMENT_NAME:-robotwin_adjust_bottle_eventvalue_full}"
...
exec python "${REPO_PATH}/examples/embodiment/train_embodied_agent.py" \
  ...
  +algorithm.event_value_source=learned_sidecar \
  +algorithm.event_boundary_threshold=0.5 \
  ...
  +algorithm.event_branch.min_supervision=8 \
  +algorithm.event_branch.influence_lr=1.0e-4 \
  +algorithm.event_credit.granularity=chunk \
  +algorithm.event_credit.max_lambda=0.0 \
  +algorithm.event_credit.min_supervision_for_actor=999999 \
  +algorithm.event_credit.require_ranking_validation=true \
  +algorithm.event_credit.ranking_validation_passed=false \
