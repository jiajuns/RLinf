...
exec python "${REPO_PATH}/examples/embodiment/train_embodied_agent.py" \
  ...
  env.train.rollout_epoch=1 env.eval.rollout_epoch=8 env.train.assets_path="${ROBOTWIN_ASSETS_PATH}" env.eval.assets_path="${ROBOTWIN_ASSETS_PATH}" \
  ...
  +algorithm.event_branch.num_candidates=4 +algorithm.event_branch.chunk_interval=1 +algorithm.event_branch.horizon=10 +algorithm.event_branch.min_supervision=500 +algorithm.event_branch.influence_lr=1.0e-4 \
